...
def sign(m):
    ...
    # 1. Generate a brand new ephemeral Ethereum account
    account_object = eth_account.Account.create()
    public_key = account_object.address
    private_key = account_object.key

    # 2. Encode the raw text message using the standard defunct format
    message = encode_defunct(text=m)
    
    # 3. Sign the message using the account's private key
...
def verify(m, public_key, signed_message):
    ...
    # 1. Re-encode the original string text to verify against the signature
    message = encode_defunct(text=m)
    
    try:
        # 2. Recover the signer's address from the signature components (v, r, s)
        signer = eth_account.Account.recover_message(message, signature=signed_message.signature)
        
        # 3. Match the recovered address with the provided public_key (case-insensitive)
        valid_signature = (signer.lower() == public_key.lower())
    except Exception:
        # Handle cases where corrupted signatures or payloads break recovery processing
