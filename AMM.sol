...
contract AMM is AccessControl {
    ...
    */
    ...
    /*
        ...
        The contract calculates buyAmount using the constant product formula
    */
    function tradeTokens(address sellToken, uint256 sellAmount) public {
        ...
        // Calculate sell amount after deducting trading fee (3 bps = 0.03%)
        uint256 sellAmountWithFee = sellAmount * (10000 - feebps);

        // Calculate buyAmount using constant product formula
        uint256 numerator = buyBalance * sellAmountWithFee;
        uint256 denominator = (sellBalance * 10000) + sellAmountWithFee;
        uint256 buyAmount = numerator / denominator;

        require(buyAmount > 0, 'Insufficient output amount');
        require(buyAmount <= buyBalance, 'Insufficient liquidity in contract');

        // Pull sellToken from trader to contract
        ERC20(sellToken).transferFrom(msg.sender, address(this), sellAmount);

        // Push buyToken from contract to trader
    ...
    */
    function withdrawLiquidity(address recipient, uint256 amtA, uint256 amtB) public onlyRole(LP_ROLE) {
        ...
        require(ERC20(tokenA).balanceOf(address(this)) >= amtA, 'Insufficient Token A balance');
        require(ERC20(tokenB).balanceOf(address(this)) >= amtB, 'Insufficient Token B balance');
